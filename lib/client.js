...
function BoxViewQueue(options) {
    var bvq = this;
    this.options = extend(true, {}, BoxViewQueue.defaults, options);
    extend(true, this.options.uploadQueue, this.options.queueOptions);
    extend(true, this.options.sessionQueue, this.options.queueOptions);
    extend(true, this.options.messageQueue, this.options.queueOptions);
    ...
    this.uploadQueue = new this.Queue('uploads', this.options.uploadQueue);
    this.uploadQueue.process(createUploader(this));
    this.sessionQueue = new this.Queue('sessions', this.options.sessionQueue);
    this.sessionQueue.process(createSessionCreator(this));
    this.messageQueue = new this.Queue('messages', this.options.messageQueue);
...
BoxViewQueue.prototype.viewURL = function (url, uploadOptions, sessionOptions) {
    ...
    function handleViewable(doc) {
        ...
        sessionEE.one('done', function (session) {
            ee.emit('done', session, doc);
        });
    }
    uploadEE.one('error', function (err) {
        ee.emit('error', err);
    });
